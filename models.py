...
def retrieve_config_and_model(
    model_name, download_dict, config_filename, checkpoint_filename
):
...
class ModelWrapper(metaclass=ABCMeta):
    def __init__(self, clip_model, device="cuda:0"):
        if clip_model not in clip.available_models():
            raise ValueError(
                f"Provided model_name = {model_name} is not available. The list of available models is {clip.available_models()}"
            )
...
class VQGAN(ModelWrapper):
    ...
    def __init__(self, model_name, clip_model, device="cuda:0"):
        if model_name not in self.model_data:
            raise ValueError(
                f'Provided model_name = "{model_name}" is not available. The list of available models is {self.available_models()}'
            )
    ...
    def get_initial_latent(self, **kwargs):
        ...
        z = torch.nn.functional.one_hot(
            torch.randint(codebook_len, [tokens_y * tokens_x]).to(self.device),
            codebook_len,
        ).float()
        z = torch.matmul(z, self.generator.quantize.embedding.weight)
        z = z.view([-1, tokens_y, tokens_x, self.generator.quantize.e_dim]).permute(
            0, 3, 1, 2
        )
    ...
    def synthesize_image(self, latent):
        ...
        return torch.clamp(
            l_decode.add(1).div(2), min=0, max=1
...
class BIGGAN(ModelWrapper):
    ...
    def get_initial_latent(self, **kwargs):
        ...
        if len(class_idx) > 0:
            ...
            class_vector = (
                class_vector * (1 - self.class_smoothing * 1000 / 999)
                + self.class_smoothing / 999
            )
        else:
            class_vector = (
                torch.ones((1, 1000), dtype=torch.float32) * self.class_smoothing / 999
            )
            class_vector[random.randint(0, 999)] = 1 - self.class_smoothing

        noise = torch.tensor(
            truncated_noise_sample(), requires_grad=True, device=self.device
        )
        class_vector = torch.tensor(class_vector, device=self.device)
        class_vector = torch.log(class_vector + 1e-8).unsqueeze(
            0
        )  # Avoid possible log(0)
    ...
    def synthesize_image(self, latent):
        ...
        noise_vector_trunc = noise_vector.clamp(
            -2 * self.truncation, 2 * self.truncation
        )
        class_vector_norm = torch.nn.functional.softmax(class_vector, dim=-1)
        l_decode = self.generator(
            noise_vector_trunc, class_vector_norm, self.truncation
        )
