...
from utils import display_tensor, set_seed, save_to_gif
from database import poll_pending_jobs, get_image_config, update_image
...
class Interpolate(torch.nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(x, size=self.size, mode=self.mode, align_corners=True)
        return x
...
def init_transforms(output_resolution):
    return torch.nn.Sequential(
        RandomHorizontalFlip(),
        RandomAffine(30., interpolation=InterpolationMode.BILINEAR),
...
def run_inference(config):    
    ...
    imgs = inference(
        ...
        imagenet_classes=config["imagenet_classes"].split(" ") if config["imagenet_classes"] is not None else [],
...
def inference(
    ...
    with torch.no_grad():
        z = model.get_initial_latent(image_shape=image_shape, imagenet_classes=imagenet_classes)
    ...
    optimizer = torch.optim.Adam([{'params':[z] if type(z) is not list else z, 'lr': learning_rate}])
        ...
        for i in range(update_freq):
            ...
            if len(text) == 1:
                ...
                for i in range(len(text)):
                    alpha = -similarity_factor if i == 0 else -(similarity_factor / 2) # Weight the first prompt twice the rest
                    loss.append(alpha * torch.cosine_similarity(embed_text[i,:], out, -1))
                loss = torch.stack(loss, dim=0).sum().div((len(text) - 1) / 2 + 1) # Take the weighting into account when computing the mean
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        if display_images:
...
def main():
    ...
    update_image(job_id, image_id, os.path.basename(gif_filename) if gif_filename is not None else None, os.path.basename(image_filename))
