import sqlite3, sys, os, time, datetime, signal
from constants import DATABASE_PATH, DATABASE_INIT_SCRIPT
...
class DB(object):
    ...
    def _query_one(self, *args):
        if len(args) > 2:
            raise RuntimeError("_query_one expects a maximum of 2 arguments: (query: str, [params: tuple])")
    ...
    def query_image_id(self, dirname, filename, colname):
        return self._query_one(f"SELECT id FROM images WHERE dirname = ? AND {colname} = ?", (dirname, filename,))
    
    
    def query_image_filenames(self, image_id):
        return self._query_one("SELECT dirname, gif_filename, image_filename FROM images WHERE id = ?", (image_id, ))
    
    
    def query_pending_jobs(self):
        return self._query_one("SELECT id, process_id, image_id FROM job_queue WHERE status == 'PENDING' ORDER BY insert_time ASC")
...
def push_job(image_config, image_filenames):
    ...
    image_filenames[0] = os.path.join(dirname, gif_filename) if gif_filename != "" and gif_filename is not None else None
    image_filenames[1] = os.path.join(dirname, img_filename) if img_filename != "" and img_filename is not None else None
    db.close()
...
def get_image_data(image_id):
    ...
    db.close()
    gif_filename = os.path.join(data[2], data[3]) if data[3] != "" and data[3] is not None else None
    image_filename = os.path.join(data[2], data[4]) if data[4] != "" and data[4] is not None else None
