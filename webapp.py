...
from models import VQGAN, BIGGAN
from utils import download_file
from database import push_job, get_random_image_id, get_image_data, process_vote, image_id_from_filename
...
def load_imagenet_dict():
    ...
    return {value[1]: int(key) for key, value in contents.items()}
    ...
    for dirname in dirs:
        for filename in os.listdir(dirname):
            if not filename.startswith("."):
                ...
                image_dict[os.path.join(dirname, split_filename[0])][0 if split_filename[1] == ".gif" else 1] = os.path.join(dirname, filename)
...
def setup_main_area():
    st.info("Work done by Xabier Lahuerta Vázquez as the Deep Learning course final project. \n\nFeel free to contact me at [xlahuerta@pm.me](mailto:xlahuerta@pm.me).")
...
def previous_inferences_page(filter_key):
    ...
    containers[0].selectbox(
        "Filter by Model",
        ["All", "Poster", "Poster Session", "VQ-GAN"] + VQGAN.available_models() + ["BigGAN"] + BIGGAN.available_models(),
        key="filter_key",
    )
    ...
    for animated_gif, final_image in retrieve_images(filter_dirs):
        ...
        prompt = re.sub("\+.*$", "", " ".join((" | ".join(basename.split("_"))).split("-")), re.M)
...
def image_voting_page():
    ...
    st.info("You can submit two votes for a given image: one corresponding to the __quality__ of the generation, and the other correspondng to the __agreement__ with the textual description.")
    ...
    if not st.session_state.quality_voted[image_id]:
        ...
        for i in range(1, 6):
            btn = columns[i - 1].button(label=f"{i}", key="quality_{i}", on_click=submit_vote, args=("quality", i,))
    else:
    ...
    if not st.session_state.agreement_voted[image_id]:
        columns = vote_container.columns(5)
        for i in range(1, 6):
            btn = columns[i - 1].button(label=f"{i}", key="agreement_{i}", on_click=submit_vote, args=("agreement", i,))
    else:
        vote_container.success("Vote successfully processed!")
...
def reset_config():
    ...
    st.session_state.similarity_factor = 10.
...
def reset_text():
    ...
    st.session_state.smoothing_factor = .1
    st.session_state.truncation_factor = 1.
...
def process_input():
    ...
    if config["model_type"] == "BigGAN":
        ...
        config["imagenet_classes"] = [IMAGENET_CLASSES[x] for x in st.session_state.imagenet_classes]
